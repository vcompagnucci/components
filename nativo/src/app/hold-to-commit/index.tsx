...
import type { Carga } from '@/piezas/hold-to-commit/carga'
import { CARGA, MEDIR, RECEPTOR, SONDA } from '@/piezas/hold-to-commit/sonda'

/* ═══════════════════════════════════════════════════════════════
   HOLD TO COMMIT — la ruta. Monta la pantalla y nada más.
   ...
   **Opal** (Screen Time Control, Apple Design Award 2025), publicado por
   ...
   Commit Button Interaction"; 60 fps. Medida cuadro a cuadro: el recibo de cada valor está en `piezas/hold-to-commit/
   medidas.ts` y los scripts de medición en `.context/hold-to-commit/`.

   FLACO A PROPÓSITO: Expo Router convierte en ruta todo `.tsx` que
   cuelgue de `src/app/`, así que el mecanismo vive en `src/piezas/`.

   La sonda (`?parcar=0.5`, `commit`, `rafaga=0.2`, `cruce=25`,
   ...
   escribiéndola en `piezas/hold-to-commit/sonda.ts` (ver ahí por qué).
   El fondo (`?fondo=liso`, o `FONDO` en `piezas/hold-to-commit/fondo.ts`)
   elige qué hay detrás del botón, y la receta (`?receta=skill`, o
   `RECETA` en `receta.ts`) qué curvas y tiempos lleva: los medidos del
   clip o los de las tablas de animate-expo. Con `'elegir'` la pieza
   muestra un selector para cambiarlos en vivo.
 ...
 *   igual. Las copias borrosas se regeneran con `media/generar.swift`.
 *
 ...
 *   `media/compra.wav` = `payment_success.caf` de iOS; ASSET DE APPLE,
