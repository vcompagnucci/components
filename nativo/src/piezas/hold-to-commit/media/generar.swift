...
// ── 1. brillo.png — el brillo ambiente en reposo ─────────────────────
...
escribir("brillo@3x.png", ancho: Int(PILL_ANCHO * ESCALA), alto: Int(PILL_ALTO * ESCALA)) { x, y in
...
// ── 2. cuerpo.png — el cuerpo del relleno, sólo perfil vertical ───────
// 3 px de ancho; React Native lo estira a lo ancho (resizeMode 'stretch').
// Opaco, con el rim verde pálido de `colorRim`.
escribir("cuerpo@3x.png", ancho: 3, alto: Int(PILL_ALTO * ESCALA)) { _, y in
    let c = colorRim(y); return (c.0, c.1, c.2, 1)
}

// ── 3. frente.png — el borde de ataque del relleno ────────────────────
...
escribir("frente@3x.png", ancho: Int(FRENTE_ANCHO * ESCALA), alto: Int(PILL_ALTO * ESCALA)) { x, y in
...
// ── 4. velo.png — la punta izquierda del blob ───────────────────────────
...
escribir("velo@3x.png", ancho: Int(VELO_ANCHO * ESCALA), alto: Int(PILL_ALTO * ESCALA)) { x, y in
...
escribir("chispa@3x.png", ancho: Int(CHISPA_CAJA * ESCALA), alto: Int(CHISPA_CAJA * ESCALA)) { x, y in
