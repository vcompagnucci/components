...
const PIEZAS_DIR = path.join(RAIZ, 'public/pieces')
...
if (alfa) {
  ...
  src = ponerCampo(src, oscuro ? 'videoOscuro' : 'video', `/pieces/${slug}${sufijo}.webm`)
  src = ponerCampo(src, oscuro ? 'videoHevcOscuro' : 'videoHevc', `/pieces/${slug}${sufijo}.mov`)
...
const bloque = src.slice(inicio, cierre)
const linea = `${campo}: '/pieces/${slug}${sufijo}.mp4',`
...
console.log(`listo — ${mb} MB. pieces.ts tiene ${campo}: '/pieces/${slug}${sufijo}.mp4'. Mirala en /${slug}`)
