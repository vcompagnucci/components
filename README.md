...
How the product works and how the work is done is in [AGENTS.md](AGENTS.md),
and that is the first thing to read. Every token with its value across the five
viewports is in [DESIGN.md](DESIGN.md). This file is why each thing is the way
it is.
...
| Text grays | one secondary level: one alpha at 37%, two bases. Annotation 160 from black, nav 166 from ink | benji declares one text color token and no gray token. Everything gray is his black or his ink at an alpha, and .4 dominates with 40 declarations against 8 for the next. With α=.4 the two bases reproduce his 152 and 159 dead on, which is what validates the model |
