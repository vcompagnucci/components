...
| ↳ tokens por rol | La tipografía dejó de vivir hardcodeada por clase: cada rol (`--t-mast-*`, `--t-body-*`, `--t-section-*`, `--t-piece-*`, `--t-meta-*`, `--t-idx-*`) es un juego de tokens. Las interlíneas van en **px**, no como razón | hizo falta para poder montar los dos sistemas candidatos, porque no son variantes del mismo: benji comparte un tamaño entre todos los roles y josh le da uno a cada uno. Se queda porque es donde vive el peso de cada rol, que si no vuelve a esconderse en su clase |
