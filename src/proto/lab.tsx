...
/* ─────────────────────────────────────────────────────────────
   ...
   NO COPIA NINGÚN VALOR. Lee del CSS servido las declaraciones
   de `:root` —las de arriba de todo para claro, las del media
   query `prefers-color-scheme: dark` para oscuro— y las aplica
   inline. O sea que muestra exactamente lo horneado y no se
   puede desincronizar de tokens.css.
...
export function Lab() {
  ...
  useEffect(() => {
    ...
    if (Number.isInteger(n) && n >= 0 && n < TEMAS.length) setT(n)
  }, [])
